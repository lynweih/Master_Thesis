...
binary_vars <- c("bilateral", "loc10", "Ascites", "Shadows", "wallreg", "papflow", "oncocenter")
#binary_vars <- c("bilateral", "loc10", "Ascites", "papflow")
...
for (i in 1:100) {
  ...
  formula<-outcome1 ~ rcs(Age, 3) + rcs(lCA125,3)  + 
    bilateral + loc10 + Ascites + papflow + colscore + oncocenter # change higher-order variable
  
  # Fit logistic regression model
  lr_list[[i]] <- lrm(formula, data = train_list[[i]])
  test_prob_list[[i]] <- data.frame(
    prob = predict(lr_list[[i]], test_list[[i]], type = "fitted"),
...
write.csv(test_prob_df, "~/master_thesis/logistic regression/simple_model/probs/test_prob_8_2_1000_small_prevalence.csv.gz", row.names = FALSE)
...
write.csv(group_metrics_df, "~/master_thesis/logistic regression/simple_model/metrics/group_metrics_8_2_1000_small_prevalence.csv", row.names = FALSE)
